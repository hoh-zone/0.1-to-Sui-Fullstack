...
import {
    ...
    addNFTToFolderTx,
    createFolderTx,
    removeCoinFromFolderTx,
    removeNFTFromFolderTx
} from "@/lib/contracts";
...
export default function Manage() {
    ...
    const {handleSignAndExecuteTransaction: handleRemoveCoin} = useBetterSignAndExecuteTransaction({
        tx: removeCoinFromFolderTx,
        waitForTx: true
    });
    const removeCoinFromFolder = async (coinType: string) => {
        if (!resource?.account || !currentFolderID)
            return;
        await handleRemoveCoin({
            account: resource.account,
            folderID: currentFolderID,
            coinType
        }).onSuccess(() => setRefresh(true)).onError((err) => console.log(err)).onExecute();
    }
    const {handleSignAndExecuteTransaction: handleRemoveNFT} = useBetterSignAndExecuteTransaction({
        tx: removeNFTFromFolderTx,
        waitForTx: true
    });
    const removeNFTFromFolder = async (nftAdd: string, nftType: string) => {
        if (!resource?.account || !currentFolderID)
            return;
                                ...
                                return (
                                    <div className="flex flex-col gap-1" key={index}>
                                        ...
                                        <div className="flex justify-between items-center">
                                            ...
                                            <div
                                                className="border rounded-full bg-[#282725] px-3 py-1 text-[#D5D4D3] cursor-pointer"
                                                onClick={() => removeCoinFromFolder(coin.coinType)}>remove
                                            </div>
                                        </div>
                                    </div>
                                )
                                ...
                                return (
                                    <div className="flex flex-col gap-1" key={index}>
                                        ...
                                        <div className="flex justify-between items-center">
                                            ...
                                            <div
                                                className="border rounded-full bg-[#282725] px-3 py-1 text-[#D5D4D3] cursor-pointer"
                                                onClick={() => removeNFTFromFolder(nft.NFTID, nft.NFTType)}>remove
                                            </div>
                                        </div>
                                    </div>
                                )
