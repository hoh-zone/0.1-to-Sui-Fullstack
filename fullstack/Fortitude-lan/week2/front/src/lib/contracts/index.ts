...
import { State, User, Profile, Folder, FolderData, SuiObject } from "@/type";
const contractName = "week_two"
...
export const queryState = async () => {
    const events = await suiClient.queryEvents({
        query: {
            MoveEventType: `${networkConfig.testnet.packageID}::${contractName}::ProfileCreated`
        }
    })
    const state: State = {
        users: []
