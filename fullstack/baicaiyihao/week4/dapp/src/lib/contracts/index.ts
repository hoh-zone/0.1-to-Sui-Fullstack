...
export const removeCoinFromFolderTx = async (folder: string, coin_type: string, sender: string)=>{
...
export const addNFTToFolderTx = async (folder: string, nft: string, nft_type: string)=>{
    if (!isValidSuiAddress(folder) || !isValidSuiAddress(nft)) {
        throw new Error("Invalid SUI address");
    }
    const tx = new Transaction();
    tx.moveCall({
...
export const removeNFTFromFolderTx = async (folder: string, nft_addr: string, nft_type: string, sender: string)=>{
    if (!isValidSuiAddress(folder)) {
        throw new Error("Invalid SUI address");
    }
    const tx = new Transaction();
    const nft = tx.moveCall({
        ...
        arguments: [
            tx.object(folder),
            tx.pure.address(nft_addr)
        ],
        typeArguments: [nft_type],
    });
    tx.transferObjects([nft], sender);
    return tx;
}