...
async function buildAndPublishPackage(
	...
	try {
		...
		const { modules = [], dependencies = [] } = output
			? JSON.parse(output)
			: {};
		...
		const objects = createdObjects.reduce<Record<string, string>>(
			(acc, item) => {
				...
				if (item.data.type === "package") {
					...
					const typeName = item.data.type
						.slice(68) // Remove package prefix
        ...
        export const ${env}: BaseContract = {${Object.keys(objects)
